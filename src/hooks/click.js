  ...
  return function (hook) {
    ...
    return hook.app.service('games').get(hook.id)
      .then((game) => {
        ...
        //   return row.row === board[boxNumber].row
        // }
        // console.log(board.find(findRow))
        // for (var row in board) {
        //   if (row === 1) {console.log(board[row])}
        // }
        ...
        board.forEach(function (object) {
            object.box === true ? trueCount++ : falseCount++;
        });

        players[0].score = trueCount
        players[1].score = 64 - trueCount
